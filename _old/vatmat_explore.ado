...
program define vatmat, rclass
	...
	use "$iotable", clear 
	merge 1:1 sector using "${tmp}\shares.dta", assert(match) nogen
	assert !mi(exempted) & !mi(exemptshare) & !mi(zerorated) & !mi(zeroratedshare)
	...
	* Define lists of sectors : exempted, mixed, and non-mixed sectors 
			...
			levelsof `sector' if `exempt'==1 & `pexempt'>0 & `pexempt'<1 ,  local(noncollsecs) // List of mixed sectors - to be expanded 
			levelsof `sector' if `pexempt'==0 | `pexempt'==1,  local(collsecs) // List of completely exempt or standard-rated - not to be expanded 
	
	
	
	dis "Program ended "
	dis "list of sectors to be expanded : `collsecs'"
	dis "list of sectors to noT be expanded : `noncollsecs'"
 ...
 --------------------------------------*/
	...
	forvalues n=1(1)`rows' {
		...
		forvalues i=1(1)`rows' {
			...
			mata: extended[jj::jj,j::k]=J(1,2,io[`n',`i']*ve[`n',1])  //multiply the original IO by the exempt share 
			mata: extended[kk::kk,j::k]=J(1,2,io[`n',`i']*nve[`n',1])  //multiply the original IO by the non-exempt share 
		
		}
		mata: exempt_i[jj,1]=1  //all odd numbers are exempt 
		mata: exempt_i[kk,1]=0  //all even numbers are not exempt 
	}
