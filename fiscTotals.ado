...
program define fiscTotals
	version 16.0
	syntax varlist(min = 1) [, pcweight(string) unit(integer 1) data(string) exportfile(string) exportsheet(string) restore]
	...
	* Concentration shares
	...
	if "`unit'" == "6"{
		replace tot = tot/1e6
		lab var tot "Survey total (Million LCU)"
	} 
	else if "`unit'" == "9"{
		replace tot = tot/1e9
		lab var tot "Survey total (Billion LCU)"
	} 
	else if "`unit'" == "12"{
		replace tot = tot/1e12
		lab var tot "Survey total (Trillion LCU)"
	} 
