cap program drop vatSimTool
program define vatSimTool
version 16.0
 ...
 **************************************************************************************
		...
		sum vatRateDir `dir' vatRateInd `ind'

		* Back out formal and informal purchases, based on gross of subsidies and excises and net of vat
		...
		* Back out formal and informal purchases, gross of VAT
		g frm_purc_gross_vat_PY = frm_purc*((1 + `dir')*(1 + `ind'))   
		g inf_purc_gross_vat_PY = inf_purc*(1 + `ind') 
		assert frm_purc_gross_vat_PY >= 0 & inf_purc_gross_vat_PY >= 0 & frm_purc_gross_vat_PY >= frm_purc & inf_purc_gross_vat_PY >= inf_purc

		* Calculate pos-VAT purchases
		egen purc_gross_vat_PY = rowtotal(frm_purc_gross_vat_PY inf_purc_gross_vat_PY)
		assert purc_gross_vat_PY >= 0 & purc_gross_vat_PY >= (frm_purc_gross_vat_PY + inf_purc_gross_vat_PY)

		* Calculate VAT 

		* Formal 
		g itx_vatx_frml_item = frm_purc*((1 + `dir') * (1 + `ind') - 1) 

		* Informal 
		g itx_vatx_infr_item = inf_purc*`ind'
		...
		* Checks
		loc j = 0
		foreach i in nx ex{
			g frm_`i'_vatrate = itx_vatx_frml_item/frm_purc_gross_vat_PY if exempted == `j'
			g inf_`i'_vatrate = itx_vatx_infr_item/inf_purc_gross_vat_PY if exempted == `j'
			loc ++j
		}
		...
		* Recalculate direct and indirect VAT such that they are additive 
		g itx_vatd_item = frm_purc * `dir' //assume that direct tax is only levied on formal purchases
		...
		* 1. No informality but exemptions 
		g itx_vatx_ninf_item = `purchases'*((1 + `dir') * (1 + `ind') - 1) 

		* No tax expenditures
		g itx_vatx_nexp_item = frm_purc* `rate' 

		* 2. No tax expenditures, nor informality 
		g itx_vatx_perf_item = `purchases' * `rate' 
